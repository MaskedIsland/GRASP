...
class GRASP(nn.Module):
    ...
    def forward(self, input):
        ...
        if in_len<self.receptive_field:
            ...
            x = input
        # print("start_conv_x.shape:", x.shape)
        x = self.start_conv(x)
        ...
        for i in range(self.blocks * self.layers):
            ...
            # print("model_filter.shape:", filter.shape)
            filter = torch.tanh(filter)
            gate = self.gate_convs[i](residual)
            # print("model_gate.shape:", gate.shape)
            ...
            # print("model_skip.shape:", skip.shape)
        ...
        x = self.end_conv_2(x)
        # print("model_final_x.shape:", x.shape)
        return x, self.nodevec1
